	...
	public void likePost(final String permalink,final int ordinal) {
		...
		if (post != null) {
			...
			for (DBObject comment : comments) {
				...
				// comentario com like
				if (contador == ordinal) {
					Map<?,?> commentMap = comment.toMap();
					int likes = (Integer) commentMap.get("num_likes");
					likes++;
					comment.put("num_likes",likes);

					// funciona, mas coloca post.num_likes
					// BasicDBObject newDocument = new BasicDBObject().append("$inc",new BasicDBObject().append("num_likes",1));

					// erro
					// BasicDBObject newDocument = new BasicDBObject().append("$inc",new BasicDBObject().append("comments.num_likes",1));
					// postsCollection.update(new BasicDBObject().append("_id",id),newDocument,true,false);

					postsCollection.update(new BasicDBObject("permalink",permalink),new BasicDBObject().append("$set",new BasicDBObject("comments",comments)),true,false);
